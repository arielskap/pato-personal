...
const Index: NextPage = () => {
	const { watch, register } = useForm<FormProps>()
	const watchAllFields = watch();

	const mailto = info.contact.email
	const subject = encodeURIComponent(`Page Contact: ${watchAllFields.subject}`)
	const message = encodeURIComponent(`Name: ${watchAllFields.name}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3">
            ...
            <div className="relative px-6 py-10 overflow-hidden bg-indigo-700 sm:rounded-lg sm:px-10 xl:p-12">
              ...
              {info.titles.map((title) => (
                <h2 key={title} className="text-sm text-white">
                  {title}
                </h2>
              ))}
              ...
              {info.courses.map((course) => (
                <h2 key={course} className="text-sm text-white">
                  {course}
                </h2>
              ))}
              <dl className="mt-8 space-y-6">
                ...
                <dd>
                  <a
                    className="flex items-center text-base text-indigo-50"
                    href={`https://wa.me/${info.contact.phone}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <PhoneIcon
                      className="flex-shrink-0 w-8 h-8 p-1 text-indigo-200 border-2 border-indigo-200 rounded-full"
                      aria-hidden="true"
                    />
